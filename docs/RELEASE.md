...
- `app.config.ts` — bundle id `com.mckelveygreg.sansapp`, the Bluetooth usage strings, and
  `ITSAppUsesNonExemptEncryption=false` (skips the export-compliance prompt). It does **not** hold a
  version literal: `version` is imported from `package.json` and `android.versionCode` is derived from
  it (see "Version numbers" below).
...
1. Bump the version in one place: `npm version 1.3.0 --no-git-tag-version` (or `npm version minor`).
   `app.config.ts` reads it from `package.json`, so iOS, Android, and the Settings page all follow.
   Build numbers need no attention — see "Version numbers" below.
...
Neither lane auto-submits — you review and press **Submit for Review** in App Store Connect. Both
lanes set the build number themselves and, on success, push an `ios/build-<n>` tag at the commit they
built (see "Version numbers").

```sh
fastlane ios status     # read-only: what's live, what's on TestFlight, and from which commit
```
...
create the initial release); after that the lanes run unattended. `versionCode` needs no manual bump —
it is derived from the version (`1.2.0` → `10200`). To upload a _second_ AAB of the same marketing
version, Play needs a strictly higher integer: `ANDROID_VERSION_CODE=10201 fastlane android beta`.

## Version numbers

Three numbers, two lifecycles. Only the first one is a decision.

| Number                      | Where it lives                               | Who sets it                                     |
| --------------------------- | -------------------------------------------- | ----------------------------------------------- |
| Marketing version (`1.2.0`) | `package.json` → imported by `app.config.ts` | You: `npm version <x.y.z> --no-git-tag-version` |
| iOS `CFBundleVersion`       | Generated `ios/` only (git-ignored)          | `beta`/`release`: TestFlight's highest + 1      |
| Android `versionCode`       | Derived in `app.config.ts`                   | Formula: `major*10000 + minor*100 + patch`      |

**Why the build number isn't in git.** `ios/` is regenerated by `expo prebuild` and git-ignored, so a
build number committed to `app.config.ts` would be a guess about what App Store Connect already
holds. The lanes ask Apple instead (`latest_testflight_build_number` + 1, across all versions), which
is the same "remote version source" convention EAS uses. The `buildNumber: "1"` in `app.config.ts` is
only the prebuild seed.

**How a build maps back to a commit.** Because that number never lands in a commit, each upload
records its own provenance:

- `GitCommit` in the shipped `Info.plist` — the SHA the archive was built from (suffixed `-dirty` if
  the tree had uncommitted changes).
- an annotated `ios/build-<n>` tag pushed at that commit, after the upload succeeds. `fastlane ios
status` prints it in the commit column.

Builds 1–14 predate this, so their commits were reconstructed from upload timestamps; they show `—`.

**Cutting the GitHub release.** The store, not the merge, is the release event: Apple can approve days
later, and a version can be superseded before it ever ships (1.1.1 existed only as TestFlight builds
11 and 12). So tag `v<version>` and write the GitHub release when App Store Connect reports
`READY_FOR_SALE` — `fastlane ios status` is the check — and point the tag at the commit the shipped
build came from, i.e. what `ios/build-<n>` already marks. This is also why the repo has no
release-please/changesets automation: both fire on merge, and a merge here isn't a release.
